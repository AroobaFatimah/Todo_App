...
export const Login = () => {
    ...
    const handleLogin = (event) => {
        ...
        if (foundUser) {
            if (foundUser.password == event.target.email.value) {
                navigate("/tasks")
            }
            else {
                    ...
                    <div className="w-full bg-white rounded-lg dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                            ...
                            <form onSubmit={handleLogin} className="space-y-4 md:space-y-6" >
                                ...
                                <div>
                                    <Input error={errorInPassword} id="password" name="password" placeholder="Password" className={"focus:outline-none bg-gray-50 text-gray-900 sm:text-sm rounded-lg block w-full p-2.5  dark:text-white"} onChange={(e) => setPassword(e.target.value)} divStyling="flex bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg  block w-full dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" />
                                </div>
