...
export const Login = () => {
    ...
    const handleLogin = (event) => {
        ...
        const foundUser = registeredUsers.find(user => user.email == event.target.email.value);
        if (foundUser) {
                    ...
                    <div className="w-full bg-white rounded-lg dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
                        <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                            ...
                            <form onSubmit={handleLogin} className="space-y-4 md:space-y-6" >
                                ...
                                <div>
                                    <Input error={errorInPassword} id="password" name="password" placeholder="Password" className={"focus:outline-none bg-gray-50 text-gray-900 sm:text-sm rounded-lg block w-full p-2.5  dark:text-white"} divStyling="flex bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg  block w-full dark:border-gray-600 dark:placeholder-gray-400 dark:text-white" />
                                </div>
                                <Button className="w-full text-white bg-black hover:bg-black-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-full text-sm px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800 mt-2"
                                    type="submit"
                                    size="large"
                                    variant="primary"
                                    text="Login"></Button>
