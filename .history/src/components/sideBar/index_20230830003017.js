...
const SideBarButton = ({ text, link, isActive, className }) => {
    return (
        <Link to={link} className={isActive ? "font-semibold flex bg-black w-1/2  px-4 py-2 pl-10 text-sm text-white rounded-full rounded-l-none mb-3" : ''} >
            {text}
        </Link>
    );
};
