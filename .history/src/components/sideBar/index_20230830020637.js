...
const SideBarButton = ({ text, link, isActive, activeIcon, inactiveIcon }) => {
    return (
        <Link to={link} link={link} className={isActive ? "font-semibold flex bg-black w-1/2  px-4 py-2 pl-10 text-sm text-white rounded-full rounded-l-none mb-3" : "text-black font-semibold flex w-1/2 px-4 py-2 pl-10 text-sm bg-white"} >
            ...
            <div className='block justify-between md:w-1/2 lg:w-1/4 mt-5 mb-5 max-md:hidden'>
                <div>
                    {sidebarButtons.map((button, index) => (
                        <SideBarButton key={index} text={button.text} isActive={button.link === activeButton} activeIcon={button.ActiveIcon} inactiveIcon={button.InactiveIcon} />
                    ))}
                </div>
                <div>
