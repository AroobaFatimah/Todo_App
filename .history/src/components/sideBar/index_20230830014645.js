            ...
            <div className='block justify-between md:w-1/2 lg:w-1/4 mt-5 mb-5 max-md:hidden'>
                <div>
                    {sidebarButtons.map((button, index) => (
                        <SideBarButton key={index} IconClr={} text={button.text} IconClr={(button.link === activeButton) ? "#fff" : "#000"} link={button.link} svg={button.svg} isActive={button.link === activeButton} />
                    ))}
                </div>
                <div>
