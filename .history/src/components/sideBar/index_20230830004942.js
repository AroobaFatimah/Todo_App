...
const SideBarButton = ({ text, link, isActive, svg, activeBtnIcon, InactiveBtnIcon }) => {
    return (
        <Link to={link} svg={svg} className={isActive ? "font-semibold flex bg-black w-1/2  px-4 py-2 pl-10 text-sm text-white rounded-full rounded-l-none mb-3" : "text-black font-semibold flex w-1/2 px-4 py-2 pl-10 text-sm bg-white"} activeBtnIcon={isActive ? "#fff" :  } >
