    ...
    return (
        <>
            <div className={divStyling}>
                <input
                    type={(simpleInputField) ? "text" : (showPassword)?"text":passwor}
                    name={name}
                    value={value}
                    id={inputID}
                ...
                />
                <Button type='button' className={`float-right font-bold text-black pr-2 ${(simpleInputField) ? "hidden" : ""}`} onClick={handleClick} text={buttonText}></Button>
