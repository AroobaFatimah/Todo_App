...
export const Tasks = ({ checked }) => {
    ...
    let ShowAlarm = () => {
        ...
        items.map(item => {
            let taskDeadline = item.deadline;
            if (!(taskDeadline instanceof Date)) {
                let deadlineDate = taskDeadline.split(", ")[0];
                let deadlineTime = taskDeadline.split(", ")[1];
                if (deadlineDate == "Today") {
                    deadlineDate = `${new Date().getFullYear()}-${new Date().getMonth()}-${new Date().getDate()}`;
                    taskDeadline = new Date(deadlineDate + "T" + deadlineTime)
                }
                if (deadlineDate == "Tomorrow") {
                    deadlineDate = `${new Date().getFullYear()}-${new Date().getMonth()}-${new Date().getDate() + 1}`;
                    taskDeadline = new Date(deadlineDate + "T" + deadlineTime)
                }
                taskDeadline = new Date(taskDeadline);
                console.log(taskDeadline)
