...
export const Tasks = ({ checked }) => {
    ...
    let ShowAlarm = () => {
        ...
        let d = new Date();
        console.log(d);
        let date = items.map(item => new Date(item.deadline));
        console.log(date);
        ...
        itemSelectedForAlarm = items.find(item => (new Date(item.deadline) == d))
    ...
    useEffect(() => {
        const intervalId = setInterval(ShowAlarm, 1000); // 60000 ms = 1 minute
        return () => {
