...
export const Tasks = ({ checked }) => {
    ...
    return (
        <>
            ...
            <div className='absolute top-0 left-0 w-full flex justify-between items-center md:hidden bg-black text-white px-5 py-3'>
                <h2>Todo App</h2>
                <div className='w-5 h-5 '>
                    <img src={Menu} onClick={() => setMobileMenu(true)}></img>
                </div>
            </div>
            <MobileMenu setMobileMenu={setMobileMenu} Mobile mobileMenu={ } />
...
function MobileMenu({ mobileMenu, setMobileMenu }) {
    return (
        <>
            <div id="defaultModal" tabindex="-1" aria-hidden="true" class={`fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full ${mobileMenu ? "" : hidden}`}>
