...
export const Tasks = ({ checked }) => {
    ...
    let ShowAlarm = () => {
        ...
        let tasksDeadlines = items.map(item => {
            const taskDeadline = item.deadline;
            const taskYear = currentDate.getFullYear();
            const taskMonth = currentDate.getMonth();
            const taskDay = currentDate.getDate();
            const taskHour = currentDate.getHours();
            const taskMinute = currentDate.getMinutes();
        })
        itemSelectedForAlarm = items.find(item => (currentDate == item.deadline))
        console.log(itemSelectedForAlarm)
        if (itemSelectedForAlarm == undefined) {
