...
export const Tasks = ({ checked }) => {
    ...
    let [showAlarm, setShowAlarm] = useState(true);
    ...
    return (
        <>
            <div
                ...
                aria-hidden="true"
                className={`backdrop-brightness-50 fixed flex justify-center items-center  w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full ${(isModalOpen ? " " : "hidden")
                    }`}
            >
...
const Alarm = ({ setRemindMeLater, setCloseAlarm, ShowAlarm, showAlarm, setShowAlarm, taskSummary, taskDesc }) => {
    ...
    return (
        <>
            ...
            <div className={`absolute right-0 max-sm:w-full  w-1/4 mt-5 mb-5 mr-3`}>
                <div id="defaultModal" tabindex="-1" aria-hidden="true" className={`max-md:ml-3 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full ${(showAlarm) ? "" : "hidden"}`}>
