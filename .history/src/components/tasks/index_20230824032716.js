...
export const Tasks = ({ checked }) => {
    ...
    let [alarmSummary, setAlarmSummary] = useState("")
    let [alarmDesc, setAlarmDesc] = useState("")
    let ShowAlarm = () => {
    ...
    return (
        <>
            ...
            <div className='flex w-3/4'>
                ...
                />
                <div>
                    <Alarm ShowAlarm={ShowAlarm} showAlarm={showAlarm} setShowAlarm={setShowAlarm} taskSummary={alarmSummary} taskDesc={alarmDesc} />
                </div>
            </div>
        </>
    )
...
const Alarm = ({ ShowAlarm, showAlarm, setShowAlarm, taskSummary = "Submit Resume", taskDesc = "Send resume to head office!" }) => {

    return (
        <>
            {console.log(ShowAlarm())}
            <div className={`absolute right-0 w-1/4 mt-5 mb-5 mr-3 ${(showAlarm) ? "" : "hidden"}`}>
                    ...
                    <div className="relative w-full max-w-2xl max-h-full">
                        <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <div className="flex justify-end bg-gray-100 p-3 space-x-2 rounded-b dark:border-gray-600">
                                <button onClick={() => setShowAlarm(false)} className='text-sm font-semibold text-black mr-3'>Skip</button>
                                <button onClick={() => setShowAlarm(false)} className='text-sm font-semibold text-black'>Remind me later</button>
                            </div>
                        </div>
                    </div>
                </div>
