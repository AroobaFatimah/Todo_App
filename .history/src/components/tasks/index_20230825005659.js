...
export const Tasks = ({ checked }) => {
    ...
    return (
        <>
            ...
            <div className='fixed absolute top-0 left-0 w-full flex justify-between items-center md:hidden bg-black text-white px-5 py-3'>
                <h2>Todo App</h2>
...
function MobileMenu({ mobileMenu, setMobileMenu }) {
    return (
        <>
            <div id="defaultModal" tabindex="-1" aria-hidden="true" class={`fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0  max-h-full ${mobileMenu ? "" : "hidden"}`}>
